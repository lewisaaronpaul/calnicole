...
function loadMusic(indexNumb) {
    ...
    musicImg.src = `static/images/${allMusic[indexNumb - 1].img}.jpg`;
...
repeatBtn.addEventListener("click", () => {
    ...
    switch (getText) {
        case "repeat":
            ...
            repeatBtn.setAttribute("title", "Repeat Song");
            break;
        case "repeat_one":
            repeatBtn.innerText = "shuffle";
            repeatBtn.setAttribute("title", "Shuffle Playlist");
            break;
        case "shuffle":
...
for (let i = 0; i < allMusic.length; i++) {
    ...
    let liAudioTag = musicList.querySelector(`.${allMusic[i].src}`);
...
progressArea.addEventListener('touchstart', handleProgress);
progressArea.addEventListener('touchmove', handleProgress);

function handleProgress(e) {
    const progressWidth = progressArea.clientWidth;
    const clickedOffsetX = e.touches[0].clientX - progressArea.getBoundingClientRect().left;
